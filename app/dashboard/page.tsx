...
// ── Dynamically import all recharts to keep initial bundle tiny ──────────────
const RevenueChart = dynamic(() => import('@/components/charts/RevenueAreaChart'), {
  ssr: false,
  loading: () => <ChartSkeleton height="h-56 sm:h-72" />,
});

const PaymentPieChart = dynamic(() => import('@/components/charts/PaymentPieChart'), {
  ssr: false,
...
export default function DashboardPage() {
  ...
  // All 4 hooks load in parallel — each shows its own skeleton state
  ...
  return (
    <div className="flex-1 flex flex-col w-full">
      ...
      {/* Show skeleton immediately while any hook is still loading */}
      {isLoading ? (
        <DashboardSkeleton />
      ) : (
        <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl w-full mx-auto">
          ...
          <div className="p-4 sm:p-6 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-4 sm:space-y-6">
            ...
            {/* Lazy-loaded chart — doesn't block initial render */}

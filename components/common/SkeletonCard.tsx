...
const Shimmer: React.FC<{ className?: string }> = ({ className = '' }) => (
  <div
    ...
    style={{ backgroundImage: 'linear-gradient(90deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.04) 50%, rgba(255,255,255,0) 100%)', backgroundSize: '200% 100%', animation: 'shimmer 1.6s infinite, pulse 2s cubic-bezier(0.4,0,0.6,1) infinite' }}
  />
);
  ...
  <div className={`${height} w-full flex flex-col gap-2 justify-end`}>
    <div className="flex items-end gap-1 h-full px-2">
      {[40, 65, 35, 80, 55, 90, 45, 70, 30, 85, 60, 75].map((h, i) => (
        <div
          key={i}
          className="flex-1 rounded-t-md animate-pulse bg-slate-800/70"
...
export const TableRowSkeleton: React.FC<{ cols?: number }> = ({ cols = 5 }) => (
  <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800/60">
    {Array.from({ length: cols }).map((_, i) => (
      <Shimmer
        key={i}
        className={`h-4 rounded ${i === 0 ? 'w-24 shrink-0' : i === cols - 1 ? 'w-16 shrink-0 ml-auto' : 'flex-1'}`}
      />
    ))}
  </div>
);
...
export const PageSkeleton: React.FC<{ rows?: number; cols?: number }> = ({ rows = 8, cols = 5 }) => (
  ...
  <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl w-full mx-auto">
    ...
    <div className="p-4 sm:p-6 rounded-3xl bg-slate-900/80 border border-slate-800 shadow-xl space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-1">
          {[48, 72, 48, 72].map((w, i) => <Shimmer key={i} className={`h-7 w-${w === 48 ? '12' : '16'} rounded-lg`} />)}
        </div>
      </div>
      <ChartSkeleton />
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
      {Array.from({ length: 3 }).map((_, i) => (
        <div key={i} className="p-4 sm:p-6 rounded-3xl bg-slate-900/80 border border-slate-800 shadow-xl space-y-4">
          <div className="space-y-1.5">
          ...
          <div className="space-y-2">
            {Array.from({ length: 4 }).map((_, j) => <Shimmer key={j} className="h-8 rounded-xl" />)}
          </div>
        </div>
      ))}
    </div>

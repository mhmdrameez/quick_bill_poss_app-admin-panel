...
const AnalyticsBarChart = lazy(() => import('@/components/charts/AnalyticsBarChart'));
const AnalyticsPieChart = lazy(() => import('@/components/charts/PaymentPieChart'));
...
export const AnalyticsContent: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'revenue' | 'products' | 'customers' | 'coupons'>('revenue');
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center gap-1.5 p-1 bg-slate-900 border border-slate-800 rounded-2xl overflow-x-auto max-w-full">
        <button
          ...
          className={`flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
            activeTab === 'revenue'
      ...
      {activeTab === 'revenue' && (
        <div className="space-y-6">
          <div className="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-4">
            ...
            <Suspense fallback={<div className="h-72 animate-pulse bg-slate-800/50 rounded-xl" />}>
              <AnalyticsBarChart
              ...
              <div className="space-y-2 text-xs">
                {productAnalytics.slowMovers.length === 0 ? (
                  ...
                  productAnalytics.slowMovers.map((p) => (
                    <div
                      key={p.id}
                      className="p-2 rounded-xl bg-slate-800/40 border border-slate-800 flex justify-between"
                    >
